...
export const protectRoute = catchAsync(async (req, res, next) => {
  ...
  if (!authorization) return next(new AppError("No token provided", 400));
...
//   };
  ...
  const verifiedToken:DecodedToken = await new Promise((res, rej) => {
    jsonwebtoken.verify(token, process.env.JWT_SECRET, (err, token: DecodedToken) => {
      return res(token);
    });
  });
