...
export const savePlaylists = async (filePaths) => {
...
export const saveCategories = async (filePaths) => {
    const rawFiles = await Promise.all(filePaths.map((el) => fs.readFile(el)));
    const cats = await Promise.all(rawFiles.map(async (el) => {
        const category = JSON.parse(el.toString());
        const randomId = crypto.randomUUID().replace(/-/gi, "");
        return {
            name: category.message,
            id: randomId,
            playlists: await Promise.all(category.playlists.items.map(async (el) => {
